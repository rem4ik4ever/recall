...
export class InMemoryProvider implements StorageProvider {
  ...
  async searchInArchiveMemory(memoryKey: string, query: string): Promise<ArchiveEntry[]> {
    const archiveMemory = this.archiveMemoryStorage.get(this.getArchiveMemoryKey(memoryKey)) || [];

    if (!archiveMemory.length) {
      return [];
    }
    ...
    const fuse = new Fuse(archiveMemory, fuseOptions);
    const searchResults = fuse.search(query);

    // Return only the items, sorted by score (best matches first)
    return searchResults.map(result => result.item);
  }

  async deleteArchiveEntry(memoryKey: string, entryId: string): Promise<void> {
    const key = this.getArchiveMemoryKey(memoryKey);
    const entries = this.archiveMemoryStorage.get(key) || [];
    this.archiveMemoryStorage.set(key, entries.filter(entry => entry.id !== entryId));
  }

  async updateArchiveEntry(memoryKey: string, entryId: string, entry: ArchiveEntry): Promise<void> {
    const key = this.getArchiveMemoryKey(memoryKey);
    const entries = this.archiveMemoryStorage.get(key) || [];
    ...
    if (index !== -1) {
      entries[index] = entry;
      this.archiveMemoryStorage.set(key, entries);
    }
  }

  async addArchiveEntry(memoryKey: string, entry: ArchiveEntry): Promise<void> {
    const key = this.getArchiveMemoryKey(memoryKey);
    const entries = this.archiveMemoryStorage.get(key) || [];
    entries.push(entry);
    this.archiveMemoryStorage.set(key, entries);
